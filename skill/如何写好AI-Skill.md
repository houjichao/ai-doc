...
| 你的情况 | 推荐阅读路径 |
|----------|-------------|
| **从没写过 Skill，想快速上手** | 一 → 二（重点看 2.5 Quick Start）→ 三 |
...
- [一、先搞清楚 Skill 是什么](#一先搞清楚-skill-是什么)
- [二、Skill 长什么样](#二skill-长什么样)
- [三、写出好 Skill 的关键技巧](#三写出好-skill-的关键技巧)
- [四、Skill 太长了怎么办——拆（模块化）](#四skill-太长了怎么办拆模块化)
- [五、一些进阶的写法](#五一些进阶的写法)
- [六、要调外部服务？MCP vs HTTP](#六要调外部服务mcp-vs-http)
- [七、安全意识：别让 Skill 变成漏洞入口](#七安全意识别让-skill-变成漏洞入口)
- [八、懒人福音：用 Skill Creator 帮你写 Skill（含工程化评估）](#八懒人福音用-skill-creator-帮你写-skill含工程化评估)
- [九、做完了怎么验证](#九做完了怎么验证)
- [十、Skill 跑不通？调试与排错指南](#十skill-跑不通调试与排错指南)
- [十一、Skill 的生命周期管理](#十一skill-的生命周期管理)
- [十二、别踩这些坑——常见反模式](#十二别踩这些坑常见反模式)
- [十三、总结：写好 Skill 的核心检查清单](#十三总结写好-skill-的核心检查清单)
- [十四、附录](#十四附录)
- [十五、参考资源](#十五参考资源)
...
> 💡 **开始写之前，先想清楚一件事**：你的 Skill 是一件事还是好几件事？如果内容预计会超过 500 行，或者包含多个独立的工作流程，建议先看"[Skill 太长了怎么办——拆（模块化）](#四skill-太长了怎么办拆模块化)"，想好结构再动笔。磨刀不误砍柴工。
...
| 方式 | 操作 |
|------|------|
| **插件市场** | CodeBuddy 插件市场搜索 `skill-creator` 一键安装 |
...
- [ ] **避免了常见反模式**：对照"[常见反模式](#十二别踩这些坑常见反模式)"逐条检查
...
### 附录 A：不同 AI 平台的 Skill 差异对照

> ⚠️ 以下信息基于各平台公开文档整理，可能随版本更新变化，请以最新官方文档为准。

| 维度 | Claude Code | CodeBuddy | Cursor |
|------|------------|-----------|--------|
| **Skill 文件名** | `SKILL.md` | `SKILL.md` | 通过 Rules / `.cursorrules` 实现类似功能 |
| **用户级目录** | `~/.claude/skills/` | `~/.codebuddy/skills/` | `~/.cursor/rules/` |
| **项目级目录** | `.claude/skills/` | `.codebuddy/skills/` | `.cursor/rules/` |
| **YAML 元数据** | 支持 name、description、metadata | 支持 name、description、metadata | 无 YAML 头信息，纯 Markdown |
| **触发方式** | 自动匹配 + `/skill` 手动 | 自动匹配 + 手动 | 基于文件匹配和 glob 规则 |
| **脚本执行** | 支持 bash/python 脚本 | 支持 bash/python 脚本 | 有限支持 |
| **MCP 集成** | 原生支持 | 原生支持 | 支持 |

> 💡 如果你的 Skill 需要跨平台使用，建议：1) SKILL.md 正文部分保持通用；2) 在 description 中同时覆盖中英文关键词；3) 避免依赖特定平台的独有功能。
